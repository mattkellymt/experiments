...
# -----------------------------------------------------------------------------
print("Generating noisy algorithmic dataset...", flush=True)
seq = [np.random.randint(0, vocab_size) for _ in range(5)]
for i in range(5, 12000):
    # 25% of the time, inject pure random noise to encourage overfitting
    if np.random.rand() < 0.25:
        seq.append(np.random.randint(0, vocab_size))
    else:
        # Complex rule requiring attention to t-3 and t-5
...
# -----------------------------------------------------------------------------
def run_experiment():
    print(f"Starting uniform-class training (Eff Batch Size: {vocab_size}, Micro-batch: {micro_batch_size}, Grad Accum Steps: {grad_accum_steps})", flush=True)
    ...
    for config_name, cfg in decay_configs.items():
        print(f"\nTraining with {config_name}...", flush=True)
        torch.manual_seed(SEED)
        
        model = NanoGPT().to(device)
        optimizer = torch.optim.AdamW(model.parameters(), lr=3e-3, weight_decay=cfg["wd"])
    ...
    plt.suptitle('Uniform Target Training on Noisy Algorithmic Data', fontsize=14, fontweight='bold', y=0.98)
    
    output_plot_path = os.path.join(os.path.dirname(__file__), "plot_uniform.png")
